  ...
  reducers: {
    handleAddToCart: (state, action) => {
      if (state.cartList.length > 0) {
        if (state.cartList.map((item) => item.id).includes(action.payload.id)) {
          let changedCartItem = state.cartList.find(
            ({ id }) => id === action.payload.id
          );
          changedCartItem.quantity = changedCartItem.quantity + 1;
          let filteredCartList = state.cartList.filter(
            (item) => item.id !== action.payload.id
          );
          state.cartList = [...filteredCartList, changedCartItem].sort((a, b) =>
            a.name.localeCompare(b.name)
          );
        } else {
          state.cartList = [...state.cartList, action.payload].sort((a, b) =>
            a.name.localeCompare(b.name)
          );
        }
      } else {
    ...
    handleCartQuantity: (state, action) => {
      if (action.payload.type === "remove-one") {
        let currentCartItem = state.cartList.find(
          ({ id }) => id === action.payload.id
        );
        if (currentCartItem.quantity === 1) {
          ...
          state.cartList = [...filteredCartList, currentCartItem].sort((a, b) =>
            a.name.localeCompare(b.name)
          );
        }
      }
...
export const { handleAddToCart, handleCartQuantity, getProductInCartQuantity } =
  cartSlice.actions;
